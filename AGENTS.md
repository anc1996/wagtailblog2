...
### 1.1 需求承接与动工铁律（先查后议，杜绝冒进）

在日常协作中，用户常以页面链接、口语化想法或零散问题开启对话（例如：“http://.../，弄些词语... 只有表情不好” 或 “有个新需求/遇到个问题”）。智能体必须严格遵循以下三步交互协议，**严禁擅自直接动工改写代码**：

1. **第一步：静默只读排查（只查不改）**：
   - 收到用户抛出的链接、想法或零散问题时，仅执行必要的只读查看（如排查模板、样式、数据模型或已有说明书）；
   - **绝对禁止在此阶段私自修改任何业务代码、执行数据库变更或盲目派发子代理编写落地代码**。
2. **第二步：意图复述与方案对齐（等用户确认）**：
   - 用 2~3 句极简人话提炼并确认用户真实痛点；
   - 给出轻量、直观的修改方案或前后效果对比（如文案、UI布局、交互逻辑）；
   - **主动明确提问并等待用户回复**（例如：“这样调整是否符合预期？确认后我再开始推进落地”）。
3. **第三步：明确授权后方可动工（触发流水线）**：
   - **只有当用户给出明确认可或动工指令**（例如：“可以”、“改吧”、“按这个做”、“全流程交付”）后，主控助手方可启动后续的子代理派发、并行开发、测试与审查流水线。
...
- **gpt-6-sol（高推理）：架构规划与方案设计核心**。担任首席架构师与部署指挥官。负责需求拆解、存储契约设计、演进方案编制（写入 说明书/）与生产部署步骤编排；
- **gemini-3.8-flash-high（高推理）：代码落地、定向测试主力与调研辅助**。负责精确代码编写、类型标注、定向测试编写与基线调研；
- **grok-4.7（高推理）：全流程对照方案审核官**。负责对抗式安全审查、并发排查与交付门禁复核；
- **对话人（用户）：终审决策与发布唯一授权人**。严禁未经用户明确同意擅自提交代码或触发生产变更。
- **子代理调度与 Token 优化准则**：
  1. **禁止使用 Astra**：所有子代理统一使用 gpt-6-sol、gemini-3.8-flash-high 或 grok-4.7；
