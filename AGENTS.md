...
- **`gemini-3.8-flash-high`（高推理）：辅助与日常维护档**。作为只读检索、文档整理或简单局部调试的轻量补充。
...
- `pm` 产品/需求：主 Agent / **`gemini-3.8-flash-high`（高推理）**；负责需求对齐、边界澄清与业务决策把关。当开发遇到业务规则或交互模糊时，由 PM 进行口径对齐或提炼决策选项请示对话人。使用 `karpathy-guidelines`，文档检索按需使用 `mineru-document-explorer`。
...
- `ops` 运维：常规部署 `gemini-3.8-flash-high`（高推理），复杂生产发布与回滚升级 `sol` 高。负责 WSL2 环境构建、Git 分支推送、生产 SSH 同步与分级服务编排，严格遵守生产分级门禁。
...
主 agent 默认使用 `gemini-3.8-flash-high`（高推理）；涉及生产数据、不可逆迁移、跨服务契约、并发一致性、安全边界、
