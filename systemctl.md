...
- 先在 WSL2 `wagtailblog-test` 环境运行 Django 定向测试（Blog 测试位于 `blog.tests.*`）、`manage.py check`、迁移检查和两个 userscript 的 `node --check`，再提交和发布。浏览器验收必须确认入口、标题、正文、OPTIONS 和实际响应；不得用 401/Token 错误误判为 CORS 失败。
