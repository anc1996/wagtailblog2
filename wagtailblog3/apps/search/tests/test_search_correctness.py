...
class SearchPublicBoundaryTests(SimpleTestCase):
    """Blog 搜索固定由独立内容索引提供，普通页面仍使用公开 QuerySet。"""
    ...
    def test_api_formatter_keeps_a_non_blog_page_result(self):
        page = MagicMock()
        page.id = 37
        page.title = "django"
        page.specific = page
        page.get_url.return_value = "/zh-hans/python/django/"
        page.content_type.model = "blogindexpage"
        page.last_published_at = None

        from search.core import format_search_results_for_api

        result = format_search_results_for_api([page])

        self.assertEqual(len(result), 1)
        self.assertEqual(result[0]["id"], 37)
        self.assertNotIn("intro", result[0])
...
class SearchPaginationAndCacheTests(SimpleTestCase):
    ...
    def test_cache_key_contains_the_implementation_version(self):
        ...
        self.assertIn("search:v5:", key)

    def test_blog_cache_key_uses_the_stable_content_namespace(self):
        key = SearchCache.get_cache_key("关键词", search_type="blog")
        self.assertIn("search:v5:", key)
        self.assertEqual(SearchCache.get_implementation_namespace("blog"), "content")
    ...
    def test_cache_clear_deletes_only_the_current_search_namespace(self):
        ...
        delete_pattern.assert_called_once_with("search:v5:*")
...
class SearchHighlightSecurityTests(SimpleTestCase):
    ...
    def test_pages_search_without_highlights_uses_the_public_queryset_backend(self):
        query_set = MagicMock()
        ...
        with (
            ...
            result = perform_search("公开关键词", "pages")
    ...
    def test_pages_search_uses_its_declared_search_fields(self):
