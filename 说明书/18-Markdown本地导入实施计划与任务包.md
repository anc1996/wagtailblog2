...
- 2026-08-20，运维手册补充：根据生产客户端使用需求，更新 `systemctl.md` 的四服务职责表，明确 maintenance Worker 负责会话组装与媒体补偿、Beat 负责过期会话与 cleanup retry 投递；新增“Markdown 导入客户端启动确认”小节，提供可复制的 active 状态与 Worker 注册任务检查命令、生产客户端地址和 Token 边界。仅更新文档，不修改生产配置、服务、数据库、MongoDB、MinIO 或 Elasticsearch；文档提交后仅同步生产工作树，不重启服务。
