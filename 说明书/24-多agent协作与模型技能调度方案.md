...
## 11. 前端按功能分工（2026-08-26）

前端员工按用户功能和验收结果分工，不按“CSS 员工/JS 员工”拆分。每条工作线可以同时修改实现该功能所需的模板、CSS、JS 和测试，但必须声明文件边界，避免两个员工同时写同一文件。

| 功能工作线 | 负责范围 | 默认模型 | 交付门禁 |
| --- | --- | --- | --- |
| 页面信息架构与内容卡片 | 首页、索引、搜索、作者、归档的标题/元信息/摘要/媒体阅读顺序 | Luna 中；跨页面契约用 Terra 中高 | DOM 顺序、长短内容、图片比例、响应式和键盘焦点 |
| 主题与内容表现 | 明暗主题、字体、颜色 token、Markdown/代码/表格/音视频/公式的可读性 | Terra 中；跨内容块冲突升级 Sol 高 | 对比度、主题切换、reduced-motion、控制台和资源加载 |
| 导航与探索交互 | 头部导航、搜索、筛选、分页、轮播、焦点返回和 URL 状态 | Luna 中；跨组件状态用 Terra 中高 | 键盘路径、Escape、返回/刷新状态、移动触控和无溢出 |
| 作者与文章阅读任务 | 作者资料、文章卡片、阅读统计、简介填充、文章页辅助栏 | 本次由 Luna 中执行；跨页面回归由 Terra 复核 | 桌面/移动布局、数据只读边界、统计展示和访问性 |
| 前端质量与回归 | 视口矩阵、亮暗主题、网络/控制台、截图证据和回滚清单 | Luna 中；发布验收 Terra 高 | `browser-skill`、Django check、相关测试、`git diff --check` |

主 agent 负责统一视觉母题、token、共享组件契约和最终整合；功能员工不得顺手重构其他工作线。若功能涉及生产数据、不可逆迁移、跨服务契约、并发或安全边界，停止普通前端分工并升级架构/审查模型。

本规则的当前实例：将“作者卡片访问统计与日期同一行”作为一条完整的作者阅读任务交给 `gpt-5.6-luna` 中推理员工；该员工可修改作者卡片模板/CSS，但不修改后端查询、数据和方案文档，主 agent 负责整合、浏览器验收和记录结果。

### 11.1 当前批次实际调度记录（2026-08-26）

- `content_readability`：`gpt-5.6-terra` 中推理，负责真实文章内容块的暗色/焦点可读性，修改视频功能所需 CSS 与模板缓存版本。
- `article_archive_experience`：`gpt-5.6-luna` 中推理，负责文章/归档公开阅读任务，发现并修复归档暗色卡片与焦点状态。
- `home_interaction_regression`：`gpt-5.6-luna` 中推理，负责首页轮播/主题交互回归，修复 Owl 初始化事件监听和 reduced-motion 状态同步。
- 主 agent：统一视觉 token、页面契约、缓存版本、测试服务和 browser-skill 证据；拒绝按 CSS/JS 文件类型重复拆工。
- 结果：三条功能线均完成测试工作树验证；后台普通表格因缺少授权会话保持待验收，不以静态检查替代真实登录后的交互证据。

### 11.2 专项验收实际结果（2026-08-26）

- 后台普通表格线使用现有合法会话完成只读验收，未猜测凭据、未提交表单；桌面/移动端表格与筛选入口无横向溢出。
- Plyr/评论线使用真实文章页完成视频对话框打开、关闭、Escape、焦点返回，以及评论排序和回复异步加载验收；未执行发布、编辑、删除、点赞等写操作。
- 三条线均按功能目标停止会话并回传证据；主 agent 将结果写入 26 号方案，保留需要写权限的操作作为独立授权批次。
