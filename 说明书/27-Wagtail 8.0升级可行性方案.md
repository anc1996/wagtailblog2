...
- 状态：已完成生产部署。生产仓库 `main` 从 `38167eac329018aaff48b9a74a49b2399d6bd921` fast-forward 至已测试提交 `d8349c8dd8546bfea11232c492bde49717d08633`，工作树干净；后续仅追加本实施记录。
- 备份与回滚源：备份目录为 `/home/source/Django/wagtail/backups/wagtailblog3-pre-wagtail8-20260827-093058`。已校验 MySQL gzip dump、MongoDB gzip archive dry-run、MinIO 610 个对象/206274864 字节、17 个 Elasticsearch 索引的 elasticdump 逻辑导出、旧代码 git bundle、配置和 service unit；SHA256 校验通过。旧 Conda 环境已克隆为 `/root/anaconda3/envs/wagtailblog-pre-w8-20260827`，Python 3.13.2、Django 5.2.8、Wagtail 7.4.3、`pip check` 均通过。
- 索引步骤：停应用、maintenance Worker、Beat 后，以 Wagtail 7.4.3 执行 `update_index --backend default`，重建 Page 1148、BlogDocument 4、BlogImage 134、Media 4（Document/Image 为 0）。六个 Wagtail default alias 原子切换到新索引；`wagtailblog-prod-content-read` 仍仅指向 `wagtailblog-prod-content-v003`，自建内容/日志 alias 未改变。原 Elasticsearch snapshot repository 因历史挂载目录缺失而不可用，本次只形成可恢复的逻辑导出，不能称为 snapshot 备份。
- 依赖与迁移：生产环境升级为 Wagtail 8.0、Django 5.2.8、DRF 3.18.0、`draftjs_exporter` 7.1.0、`modelsearch` 1.3.2、`django-ninja` 1.6.3、`swapper` 1.4.0，`pip check` 通过。发现的四个 Wagtail 7.x 孤立 migration 及对应 pyc 已移入上述备份目录的 `wagtail-stale-migrations-from-production-env`，Wagtail 8 RECORD 校验无差异。生产迁移计划仅有 `wagtailcore.0098_apitoken`；SQL 审阅确认只创建 API token 表及用户外键，已执行成功。`0096/0097` 已有记录且只包含索引。`collectstatic --noinput` 完成 115 个复制、1370 个未修改、837 个 post-process 文件。
- 服务与验收：按 Django/uWSGI → maintenance Worker → Beat 顺序恢复；四个服务均 active/enabled，`systemctl --failed` 为 0，socket 正常。生产 vhost `192.168.20.2:6050` 首页 302、`/zh-hans/` 200、`/admin/login/` 200、管理静态 CSS 200；`browser-skill` 桌面与 390px 移动快照可读且移动 `scrollWidth == clientWidth`。production env 下 Celery maintenance ping/registered 正常；Outbox 20、Delivery 29 全部 succeeded；Page 1148、Revision 1783、API token 0；`/api/v3/` 未注册。近 20 分钟四个 unit 日志无 error/exception。
- 数据与服务影响：仅写入 Wagtail 8 API token 表并重建 Wagtail default 索引；未修改 BlogPage/StreamField、Markdown `markdown_block` key、Revision 指针、MongoDB 正文/草稿、MinIO 媒体或自建 content serving alias。`systemctl.md` 已在升级提交中补充 Wagtail 8 runbook，本批未修改 unit 配置，不需要 daemon-reload。
- 测试与门禁：WSL2 隔离测试的定向回归、fresh database 全迁移和 `compileall`/`git diff --check` 已通过；完整测试中的 3 个 app-label 导入错误和 2 个静态版本/日志约定断言为既有基线，不归因于 Wagtail 8。生产 `manage.py check` 通过；MySQL 条件唯一约束 `models.W036` 为既有能力警告。生产数据操作、索引写入、依赖安装、迁移和服务重启均在用户部署授权范围内执行。
- 回滚点：若后续故障，停止本批 app/Worker/Beat，恢复代码至 `38167eac329018aaff48b9a74a49b2399d6bd921`，将失败环境留存后用 `/root/anaconda3/envs/wagtailblog-pre-w8-20260827` 重建 systemd 固定的 `/root/anaconda3/envs/wagtailblog` 前缀，再按 runbook 启动并复验。`0098` 为新增表，默认不回滚 schema；不得删除 MongoDB、MinIO、ES serving 索引或正文数据。剩余风险为单节点 Elasticsearch yellow/14 个未分配副本（既有状态）、第三方扩展在完整低权限后台流程中的长期兼容性，以及 snapshot repository 尚未修复。
- 实际模型/技能：生产迁移、索引、备份、服务和回滚由主 agent 使用 `gpt-5.6-sol` 高推理门禁执行；`ops` 与 `review` 员工只读复核环境和回滚步骤，其中 `ops` 的轮询脚本曾误含幂等 `systemctl start`，已记录并停止其后续操作；浏览器验收改用 `browser-skill`，未向外部模型发送源码、凭据或正文。
